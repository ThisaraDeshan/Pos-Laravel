            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (true) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getAgingOne']),
                                __('lang_v1.aging_report'),
                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'aging-one']
                            );
                        }

                         if (true) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'creditReport']),
                                __('lang_v1.credit_report'),
                                ['icon' => 'fa fas fa-file-invoice-dollar', 'active' => request()->segment(2) == 'credit-report']
                            );
                        }
