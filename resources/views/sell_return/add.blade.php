					...
					<table class="table bg-gray" id="sell_return_table">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<input type="text" name="products[{{$loop->index}}][unit_price_inc_tax]" value="{{@num_format($sell_line->unit_price_inc_tax)}}" class="unit_price_inc_tax"   >
								
								
									<div class="return_subtotal">
									</div>
								</td>
							</tr>
							@endforeach
...
$(document).on('change', 'input.unit_price_inc_tax, #discount_amount, #discount_type', function() {
		update_sell_return_total_sales()
	});

	function update_sell_return_total_sales() {
	var net_return = 0;
		$('table#sell_return_table tbody tr').each(function() {
			var subtotal = __read_number($(this).find('input.unit_price_inc_tax'));
		
			var unit_price = __read_number($(this).find('input.unit_price'));
				
			var quantity = subtotal / unit_price;

			$(this).find('.return_qty')[0].value=quantity;
			console.log($(this).find('.return_qty')[0].value);
	...
	function update_sell_return_total() {
		...
		console.log( concat( net_return , ':unit',unit_price ,'subtotal',subtotal,'quantity',quantity));
		var discount = 0;
